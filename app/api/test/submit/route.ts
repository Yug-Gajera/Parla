...
export async function POST(req: Request) {
    try {
        ...
        // Score objective questions (reading, vocabulary, grammar)
        ...
        try {
            const completion = await getOpenAI().chat.completions.create({
                model: 'gpt-4o',
                messages: [{ role: 'user', content: writingPromptText }],
        ...
        // Reading: 25%, Vocabulary: 25%, Grammar: 15%, Writing: 35%
        ...
        // If passed, update user's level
        if (passed) {
            // Update user_languages to the new level they passed into
            const { error: levelUpdateError } = await (supabase as any)
                .from('user_languages')
                .update({
                    current_level: level,
                    level_score: 0, // Reset score for new level
                })
                .eq('user_id', user.id)
            ...
            // Log a study session for the test
            await (supabase as any)
                ...
                .insert({
                    ...
                    session_type: 'test',
                    duration_minutes: 30, // Approximate test duration
                    xp_earned: 100, // Bonus for passing
                });
        }

        // Return the full scored results
        return NextResponse.json({
            ...
            sections: {
                reading: {
                    score: readingPercent,
                    points_earned: readingScore,
                    points_possible: readingTotal,
                    details: readingDetails,
                },
                vocabulary: {
                    score: vocabularyPercent,
                    points_earned: vocabularyScore,
                    points_possible: vocabularyTotal,
                    details: vocabularyDetails,
                },
                grammar: {
                    score: grammarPercent,
                    points_earned: grammarScore,
                    points_possible: grammarTotal,
                    details: grammarDetails,
                },
                writing: {
                    score: writingPercent,
                    breakdown: {
                        grammar: writingScoreResult.grammar,
                        vocabulary: writingScoreResult.vocabulary,
                        coherence: writingScoreResult.coherence,
                        task_completion: writingScoreResult.task_completion,
                    },
